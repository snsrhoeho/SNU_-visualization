...
러닝코스 6개는 2026-08-18 기준 OpenStreetMap 보행자 라우팅망에 전체 좌표를 다시 매칭했습니다. 지도 팝업은 검증된 위·경도 경로를 네이버 지도에 선으로 표시하고, 같은 경로 위에서 러너 마커를 한 번 자동 재생합니다. `움직임 다시 보기`를 누르면 같은 동선을 다시 확인할 수 있습니다.

지도 검증은 물·건물 위를 직선으로 잇는 오류를 피하고 보행 가능한 네트워크에 연결됐는지 확인하기 위한 것입니다. 공사, 임시 통제, 야간 조명, 현장 출입 제한까지 보장하지 않으므로 화면의 출발 전 확인 안내를 유지해야 합니다. 배곧 코스의 짧은 계단, 오이도 코스의 램프·생활도로, 대야 코스의 다수 횡단보도도 상세 팝업에 표시합니다.
